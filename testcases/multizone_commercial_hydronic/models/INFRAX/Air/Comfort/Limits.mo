...
equation
  ...
  THardUp = if (clock.hour >= 7 and clock.hour <= 18) and (clock.weekDay) < 6 then ctrlLimits.limValue[typeLimit, 1] else ctrlLimits.limValue[typeLimit, 1] + 5;
  TSoftUp = if (clock.hour >= 7 and clock.hour <= 18) and (clock.weekDay) < 6 then ctrlLimits.limValue[typeLimit, 2] else ctrlLimits.limValue[typeLimit, 2] + 5;
  TSoftDown = if (clock.hour >= 7 and clock.hour <= 18) and (clock.weekDay) < 6 then ctrlLimits.limValue[typeLimit, 3] else ctrlLimits.limValue[typeLimit, 3] - 5;
  THardDown = if (clock.hour >= 7 and clock.hour <= 18) and (clock.weekDay) < 6 then ctrlLimits.limValue[typeLimit, 4] else ctrlLimits.limValue[typeLimit, 4] - 5;
