...
package SubControllers "Individual controller modules"
  ...
  equation
    ...
    connect(booleanToReal.u, or1.y) annotation (Line(points={{84.6,93},{84.6,43},
            {82.7,43}}, color={255,0,255}));
    connect(and1.u2, signalBus.TABSheatingDemand) annotation (Line(points={{44.6,
            45.4},{44.6,22},{100,22},{100,0}},            color={255,0,255}),
        Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}}));
    connect(and2.u2, signalBus.TABScoolingDemand) annotation (Line(points={{0.6,
            15.4},{0.6,0},{100,0}},       color={255,0,255}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false), graphics),
  ...
  model AHU_baseline "Controls of AHU loop for baseline"
    ...
    Modelica.Blocks.Tables.CombiTable1D curve_AHU(table=[273.15 + 15,273.15 +
          23; 273.15 + 21,273.15 + 17.75; 273.15 + 23,273.15 + 19; 273.15 + 24,
          273.15 + 20; 273.15 + 25,273.15 + 20.5; 273.15 + 26,273.15 + 19.5])
                              "Infrax AHU heating/cooling curve"
      annotation (Placement(transformation(extent={{-10,166},{10,186}})));
    Modelica.Blocks.Nonlinear.Limiter limiter(uMax=273.15 + 24, uMin=273.15 + 15)
    ...
    IDEAS.Controls.Continuous.LimPID con3wayCoo(
      ...
      Ti(displayUnit="s") = 40,
      reverseActing=false)
                          "Controller for 3-way valve of AHU cooling coil"
    ...
    IDEAS.Controls.Continuous.LimPID con3wayHea(
      ...
      Ti(displayUnit="s") = 40,
      k=2,
      reverseActing=true)  "Controller for 3-way valve of AHU heating coil"
    ...
    Modelica.Blocks.Sources.RealExpression
           ...
           <= 19) and (clock.weekDay) < 6) or nightVentilation == true then 180
    ...
    Modelica.Blocks.Sources.RealExpression
           ...
           <= 19) and (clock.weekDay) < 6) or nightVentilation == true then 125
           else 0)
    ...
    Modelica.Blocks.Sources.IntegerExpression P08P09signal(y=if (clock.hour >=
          6 and clock.hour <= 19) and (clock.weekDay) < 6 and switch3.y > 0
           then P0809_signal.y else 0)
    ...
    Modelica.Blocks.Sources.IntegerExpression P04signal(y=if (clock.hour >= 6
           and clock.hour <= 19) and (clock.weekDay) < 6 and switch1.y > 0
           then P04_signal.y else 0)
      "pump 04 signal"
      annotation (Placement(transformation(extent={{66,-48},{88,-28}})));
    Modelica.Blocks.Sources.RealExpression
                                     P13signal(y=if (clock.hour >= 6 and clock.hour
           <= 19) and (clock.weekDay) < 6 and switch1.y > 0 then P13_nom.y
           else 0)
      "Pump 13 signal"
      annotation (Placement(transformation(extent={{52,2},{74,22}})));
    ...
    Modelica.Blocks.Sources.Constant on(k=1)
      annotation (Placement(transformation(extent={{-32,6},{-24,14}})));
    IDEAS.Buildings.Components.Interfaces.WeaBus weaBus(numSolBus=6)
      annotation (Placement(transformation(extent={{-72,160},{-52,180}})));
    Modelica.Blocks.Logical.Not not1
      annotation (Placement(transformation(extent={{32,-252},{52,-232}})));
    Modelica.Blocks.Logical.Hysteresis hysteresis(
      pre_y_start=true,
      uLow=0,
      uHigh=0.25)       "hysteresis logic for HPs"
      annotation (Placement(transformation(extent={{-10,-226},{10,-206}})));
    Modelica.Blocks.Math.Add add(k2=-1)
      annotation (Placement(transformation(extent={{-40,-226},{-20,-206}})));
    Modelica.Blocks.Logical.Hysteresis hysteresis1(
      pre_y_start=true,
      uLow=-0.25,
      uHigh=0)          "hysteresis logic for HPs"
      annotation (Placement(transformation(extent={{-10,-252},{10,-232}})));
    Modelica.Blocks.Sources.Constant closed(k=0)
      "setpoint both for heating and cooling"
      annotation (Placement(transformation(extent={{20,-18},{32,-6}})));
  ...
  equation
    ...
    connect(P04signal.y, signalBus.P04_signal) annotation (Line(points={{89.1,
        ...
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(P13signal.y, signalBus.P13_signal) annotation (Line(points={{75.1,12},
            {100,12},{100,0}},     color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    ...
    connect(con3wayCoo.y,greaterThreshold1. u) annotation (Line(points={{11,48},
            {8,48},{8,-10},{-42,-10},{-42,-30},{-22,-30}},
                                                       color={0,0,127}));
    connect(greaterThreshold1.y, P04_signal.u)
      annotation (Line(points={{1,-30},{36,-30}},          color={255,0,255}));
    connect(on.y, P13_nom.u) annotation (Line(points={{-23.6,10},{-12,10},{-12,
            8},{-2,8}}, color={0,0,127}));
    connect(curve_AHU.u[1], dataBus.Tavg_building) annotation (Line(points={{
            -12,176},{-99.9,176},{-99.9,0.1}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-6,3},{-6,3}},
        horizontalAlignment=TextAlignment.Right));
    connect(sim.weaBus,weaBus)  annotation (Line(
        points={{-81,193},{-78,193},{-78,170},{-62,170}},
        color={255,204,51},
        thickness=0.5));
    connect(feedback.u1, weaBus.Te) annotation (Line(points={{-40,138},{-61.95,
            138},{-61.95,170.05}}, color={0,0,127}));
    connect(hysteresis.u,add. y)
      annotation (Line(points={{-12,-216},{-19,-216}},      color={0,0,127}));
    connect(hysteresis1.y,not1. u)
      annotation (Line(points={{11,-242},{30,-242}}, color={255,0,255}));
    connect(dataBus.TAirSupplyAHU,add. u1) annotation (Line(
        points={{-99.9,0.1},{-99.9,-104},{-100,-104},{-100,-210},{-42,-210}},
        color={255,204,51},
        thickness=0.5), Text(
        string="%first",
        index=-1,
        extent={{-3,6},{-3,6}},
        horizontalAlignment=TextAlignment.Right));
    connect(add.u2, con3wayCoo.u_s) annotation (Line(points={{-42,-222},{-72,
            -222},{-72,48},{-12,48}}, color={0,0,127}));
    connect(add.y,hysteresis1. u) annotation (Line(points={{-19,-216},{-16,-216},
            {-16,-242},{-12,-242}}, color={0,0,127}));
    connect(hysteresis.y, switch1.u2) annotation (Line(
        points={{11,-216},{32,-216},{32,32},{50,32}},
        color={255,0,255},
        visible=false));
    connect(not1.y, switch3.u2) annotation (Line(
        points={{53,-242},{53,-62},{58,-62}},
        color={255,0,255},
        visible=false));
    connect(closed.y,switch1. u3) annotation (Line(points={{32.6,-12},{42,-12},
            {42,24},{50,24}}, color={0,0,127}));
    connect(closed.y,switch3. u3) annotation (Line(points={{32.6,-12},{46,-12},
            {46,-54},{58,-54}}, color={0,0,127}));
    connect(con3wayCoo.y, switch1.u1) annotation (Line(points={{11,48},{30,48},
            {30,40},{50,40}}, color={0,0,127}));
    connect(switch1.y, signalBus.CooCoi_3way_signal) annotation (Line(points={{
            73,32},{100,32},{100,0}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(con3wayHea.y, switch3.u1) annotation (Line(points={{13,-70},{36,-70},
            {36,-70},{58,-70}}, color={0,0,127}));
    connect(switch3.y, signalBus.HeaCoi_3way_signal) annotation (Line(points={{
            81,-62},{100,-62},{100,0}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(con3wayCoo.u_s, con3wayTW.u_s) annotation (Line(points={{-12,48},{
            -72,48},{-72,110},{-28,110}}, color={0,0,127}));
    connect(dataBus.TAirSupplyAHU, con3wayTW.u_m) annotation (Line(
        points={{-99.9,0.1},{-99.9,88},{-16,88},{-16,98}},
        color={255,204,51},
        thickness=0.5), Text(
        string="%first",
        index=-1,
  ...
  package Pumps "Controllers of Hydraulic Pumps"
    model P1 "hydraulic pumps logic"
      ...
      Modelica.Blocks.Nonlinear.Limiter limiter(uMax=1, uMin=0)
        annotation (Placement(transformation(extent={{10,40},{30,60}})));
      Modelica.Blocks.MathBoolean.Or  Off(nu=1) "multiple off conditions"
    ...
    equation
      ...
      connect(HPIsOn.y, Off.u[1]) annotation (Line(points={{33,0},{38,0},{38,60},
    ...
    model P3 "hydraulic pumps logic"
      Modelica.Blocks.Tables.CombiTable1D P3_HP_off(table=[-5,3*9806.38; 5,10*
            9806.38])
      ...
      Modelica.Blocks.Nonlinear.Limiter limiter(uMin=6*9806.38,
                                                        uMax=12*9806.38)
        annotation (Placement(transformation(extent={{12,-10},{32,10}})));
    equation
    ...
    equation
      connect(signalBus.AHUheatingDemand, booleanToReal.u) annotation (Line(
          points={{100,0},{100,0},{100,60},{-80,60},{-80,30},{-62,30}},
          color={255,204,51},
          thickness=0.5), Text(
          string="%first",
          index=-1,
      ...
      connect(p09.y, signalBus.P09_mod) annotation (Line(points={{13,30},{54,30},{54,
          ...
          index=1,
          extent={{6,3},{6,3}}));
      connect(p13.y, signalBus.P13_mod) annotation (Line(points={{39,-30},{54,-30},{
              54,0},{100,0}},       color={0,0,127}), Text(
          string="%second",
          index=1,
          extent={{6,3},{6,3}}));
      connect(p08.y, signalBus.P08_mod) annotation (Line(points={{11,0},{100,0},{100,
              0}},   color={0,0,127}), Text(
          string="%second",
          index=1,
          extent={{6,3},{6,3}}));
      connect(signalBus.AHUheatingDemand, booleanToReal2.u) annotation (Line(
          points={{100,0},{100,0},{100,62},{100,60},{-80,60},{-80,0},{-62,0}},
          color={255,204,51},
          thickness=0.5), Text(
          string="%first",
          index=-1,
      ...
      connect(signalBus.AHUcoolingDemand, switch1.u2) annotation (Line(
          points={{100,0},{100,0},{100,-72},{-80,-72},{-80,-30},{-16,-30}},
          color={255,204,51},
          thickness=0.5), Text(
          string="%first",
          index=-1,
      ...
      connect(signalBus.CooCoi_3way_signal, P13_table.u[1]) annotation (Line(
          points={{100,0},{100,-54},{-94,-54},{-94,-20},{-79.6,-20}},
          color={255,204,51},
          thickness=0.5), Text(
          string="%first",
          index=-1,
    ...
    equation
      ...
      connect(signalBus.HP_onoff, switch1.u2) annotation (Line(
          points={{100,0},{100,20},{46,20},{46,0},{50,0}},
          color={255,204,51},
          thickness=0.5), Text(
          string="%first",
          index=-1,
      ...
      connect(realExpression.y, signalBus.P0102_mod) annotation (Line(points={{61,64},
    ...
    package RBCBaseline
      model P5_baseline "hydraulic pumps logic p05"
        ...
        Modelica.Blocks.Tables.CombiTable1D cooCurve_TABS(table=[273.15 + 10,
              273.15 + 19; 273.15 + 20,273.15 + 21.5; 273.15 + 30,273.15 + 23])
          "Infrax TABS cooling curve"
          annotation (Placement(transformation(extent={{-64,40},{-44,60}})));
        Modelica.Blocks.Nonlinear.Limiter limiter(uMax=273.15 + 23, uMin=273.15
               + 18.5)
  ...
  package Valves
    ...
    equation
      ...
      connect(add.y, signalBus.PC_3way_signal) annotation (Line(points={{27,0},{
              100,0},{100,0}},        color={0,0,127}), Text(
          string="%second",
          index=1,
          extent={{6,3},{6,3}}));
      annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
    ...
    equation
      connect(con3WayHea.y, signalBus.HeaCoi_3way_signal) annotation (Line(
            points={{27,36},{60,36},{60,0},{100,0}},       color={0,0,127}),
          Text(
          string="%second",
          index=1,
          extent={{6,3},{6,3}}));
      connect(con3WayCoo.y, signalBus.CooCoi_3way_signal) annotation (Line(
            points={{27,-10},{60,-10},{60,0},{100,0}},       color={0,0,127}),
          Text(
          string="%second",
          index=1,
          extent={{6,3},{6,3}}));
      ...
      connect(add1.y, signalBus.T_AHU) annotation (Line(points={{-1.4,36},{0,36},
              {0,66},{100,66},{100,0}},       color={0,0,127}), Text(
          string="%second",
          index=1,
          extent={{6,3},{6,3}}));
      connect(sim.weaBus, weaBus) annotation (Line(
    ...
    package BaseClasses
      ...
      model TwoWayValveCCAbaseline
        ...
        Modelica.Blocks.Tables.CombiTable1D floorSetPoint(table=[0.25,1.5; 0.5,
              1; 1,0.5])
          "TABS temperature difference setpoint (per floor)"
    ...
    package RBCBaseline
      ...
      model ThreeWayValveTABS_baseline
        ...
        Modelica.Blocks.Tables.CombiTable1D heaCurve_TABS(table=[273.15 - 8,
              273.15 + 23.5; 273.15,273.15 + 22.5; 273.15 + 5,273.15 + 21.75;
              273.15 + 10,273.15 + 21])
      ...
      equation
        ...
        connect(limiter.u, heaCurve_TABS.y[1])
          annotation (Line(points={{-38,50},{-43,50}}, color={0,0,127}));
  ...
  model NightVentilation "night ventilation RBC"
    ...
    SignalBus signalBus annotation (Placement(transformation(extent={{-122,-92},
              {-82,-52}}),
  ...
  equation
    ...
    connect(no_nv.outPort[1], T1.inPort)
      annotation (Line(points={{64.2,60},{64.2,40},{58.4,40}},
                                                             color={0,0,0}));
    connect(nv1.y, T1.condition) annotation (Line(points={{21.5,60},{40.75,60},{40.75,
            35.2},{60,35.2}},        color={255,0,255}));
    connect(T1.outPort, nv_1.inPort[1])
      annotation (Line(points={{60.6,40},{55.6,29.5},{55.6,20}},
                                                           color={0,0,0}));
    connect(nv_1.outPort[1], T2.inPort)
      annotation (Line(points={{64.2,20},{64.2,8},{64.2,0},{58.4,0}},
                                                  color={0,0,0}));
    connect(T2.outPort, stab.inPort[1])
      annotation (Line(points={{60.6,0},{55.6,0},{55.6,-20}},
                                                          color={0,0,0}));
    connect(stab.outPort[1], T3.inPort)
      annotation (Line(points={{64.2,-19.9},{58.4,-30},{58.4,-40}},
                                                              color={0,0,0}));
    connect(dataBus.Tavg_building, hysteresis2.u) annotation (Line(
        ...
        index=-1,
        extent={{-6,3},{-6,3}}));
    connect(nv2.y, T3.condition) annotation (Line(points={{21.5,40},{30,40},{30,-44.8},
            {60,-44.8}},        color={255,0,255}));
    connect(T3.outPort, nv_2.inPort[1])
      annotation (Line(points={{60.6,-40},{55.6,-50.5},{55.6,-60}},
                                                              color={0,0,0}));
    connect(nv2.y, not1.u) annotation (Line(points={{21.5,40},{30,40},{30,-30},
            {38.8,-30}}, color={255,0,255}));
    connect(not1.y, T4.condition)
      annotation (Line(points={{52.6,-30},{66,-30},{66,-34.8},{80,-34.8}},
                                                     color={255,0,255}));
    connect(T4.outPort, wait.inPort[1])
      annotation (Line(points={{80.6,-30},{80.6,-79.8},{85.6,-79.8}},
                                                            color={0,0,0}));
    connect(wait.outPort[1], T5.inPort) annotation (Line(points={{94.2,-80},{94.2,
            -100},{138.4,-100},{138.4,-84}},
                                         color={0,0,0}));
    connect(T5.outPort, no_nv.inPort[1]) annotation (Line(points={{140.6,-84},{140.6,
            -84},{140.6,72},{55.6,72},{55.6,60}},
                                                color={0,0,0}));
    connect(not2.y, T2.condition)
      annotation (Line(points={{50.6,0},{56,0},{56,-4.8},{60,-4.8}},
                                                 color={255,0,255}));
    connect(nv1.y, not2.u) annotation (Line(points={{21.5,60},{34,60},{34,0},{
            36,0},{36.8,0}},                      color={255,0,255}));
    connect(not3.y, T6.condition) annotation (Line(points={{50.6,-76},{60,-76},{60,
            -80.8}},        color={255,0,255}));
    connect(nv2.y, not3.u) annotation (Line(points={{21.5,40},{28,40},{28,-76},
            {36.8,-76}}, color={255,0,255}));
    connect(T6.inPort, nv_2.outPort[1]) annotation (Line(points={{58.4,-76},{58.4,
            -60},{64.2,-60}},   color={0,0,0}));
    connect(T6.outPort, wait.inPort[2]) annotation (Line(points={{60.6,-76},{60.6,
            -80.2},{85.6,-80.2}},    color={0,0,0}));
    connect(stab.outPort[2], T4.inPort) annotation (Line(points={{64.2,-20.1},{70,
            -20.1},{70,-30},{78.4,-30}},
                                       color={0,0,0}));
    connect(nv_1.active, or1.u1)
      annotation (Line(points={{60,15.6},{66,15.6},{66,20},{72.8,20}},
                                                      color={255,0,255}));
    connect(nv_2.active, or1.u2) annotation (Line(points={{60,-64.4},{70,-64.4},{70,
            15.2},{72.8,15.2}},          color={255,0,255}));
    ...
    connect(add.u2, signalBus.baseSetpoint) annotation (Line(points={{-62,4},{
            -74,4},{-74,-72},{-102,-72}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-6,3},{-6,3}},
        horizontalAlignment=TextAlignment.Right));
  ...
  model SunShading "sunshading control"
    DataBus dataBus annotation (Placement(transformation(extent={{-120,-20},{
              -80,20}}), iconTransformation(extent={{-110,-10},{-90,10}})));
    Modelica.Blocks.Sources.BooleanExpression east_on(y=if clock.hour > 5 and
    ...
    Modelica.Blocks.Math.Add sum2nd3rd
    ...
    Modelica.Blocks.Logical.Hysteresis hysteresis1(uLow=273.15 + 20.5, uHigh=
          273.15 + 22)
      "External temperature for which the shading device can be on"
      annotation (Placement(transformation(extent={{-28,6},{-8,26}})));
    Modelica.Blocks.Logical.Hysteresis hysteresis2(uLow=273.15 + 20.5, uHigh=
          273.15 + 22)
      "External temperature for which the shading device can be on"
  ...
  equation
    ...
    connect(sim.weaBus, weaBus) annotation (Line(
        ...
        color={255,204,51},
        thickness=0.5));
    connect(sum2nd3rd.y, avg.u) annotation (Line(points={{-55.3,-11},{-53.65,-11},
            {-53.65,-10},{-50.8,-10}}, color={0,0,127}));
    connect(sum2nd3rd.u1, dataBus.T3rdFlr_SZ) annotation (Line(points={{-71.4,-6.8},
            {-81.7,-6.8},{-81.7,0.1},{-99.9,0.1}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-6,3},{-6,3}},
        horizontalAlignment=TextAlignment.Right));
    connect(sum2nd3rd.u2, dataBus.T2ndFlr_SZ) annotation (Line(points={{-71.4,-15.2},
            {-99.9,-15.2},{-99.9,0.1}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-6,3},{-6,3}},
        horizontalAlignment=TextAlignment.Right));
    connect(avg.y, hysteresis.u)
      annotation (Line(points={{-41.6,-10},{-30,-10}}, color={0,0,127}));
    connect(hysteresis.y, west.u[2]) annotation (Line(points={{-7,-10},{4,-10},
            {4,42},{40,42},{40,46.5}}, color={255,0,255}));
    connect(hysteresis.y, south.u[2]) annotation (Line(points={{-7,-10},{4,-10},
            {4,64},{40,64},{40,66.5}}, color={255,0,255}));
    connect(hysteresis1.u, dataBus.T2ndFlr_NZ) annotation (Line(points={{-30,16},
            {-99.9,16},{-99.9,0.1}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-6,3},{-6,3}},
        horizontalAlignment=TextAlignment.Right));
    connect(hysteresis1.y, east2nd.u[2]) annotation (Line(points={{-7,16},{4,16},
            {4,86.5},{40,86.5}}, color={255,0,255}));
    connect(hysteresis2.u, dataBus.T3rdFlr_MTR) annotation (Line(points={{-60,
            30},{-99.9,30},{-99.9,0.1}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{-6,3},{-6,3}},
        horizontalAlignment=TextAlignment.Right));
    connect(hysteresis2.y, east3rd.u[2]) annotation (Line(points={{-37,30},{2,
            30},{2,106.5},{40,106.5}}, color={255,0,255}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
          coordinateSystem(preserveAspectRatio=false)));
  end SunShading;
  ...
  model VAV_baseline "Controls of AHU loop for baseline"
    ...
    Modelica.Blocks.Logical.Hysteresis hysteresisHea[21](uHigh=-1, uLow=-1.5)
      annotation (Placement(transformation(extent={{-60,60},{-40,80}})));
    Modelica.Blocks.Logical.Not not1[21]
      annotation (Placement(transformation(extent={{-28,64},{-16,76}})));
    Modelica.Blocks.Tables.CombiTable1D supplySetpoint[21](each table=[-1.5,
          273.15 + 24; -1,273.15 + 24; 0,273.15 + 18])
                        "setpoint for the supply air temperature"
      annotation (Placement(transformation(extent={{-10,20},{10,40}})));
    Modelica.Blocks.Nonlinear.Limiter limiter[21](uMin=273.15 + 16, uMax=273.15 +
          24) "air supply temperature setpoint"
    ...
    Modelica.Blocks.Sources.RealExpression flush[15](y=if (clock.hour >= 6 and
          clock.hour <= 7) or nightVentilation then 1 else 0)
    ...
    Modelica.Blocks.Routing.Replicator replicator2(nout=15)
          ...
          rotation=0,
          origin={-44,4})));
    Modelica.Blocks.Logical.Hysteresis hysteresisVAVHea[15](uHigh=-1, uLow=-1.5)
    ...
    Modelica.Blocks.Logical.Or     or2[15]
      annotation (Placement(transformation(extent={{-32,92},{-20,104}})));
    Modelica.Blocks.Logical.Hysteresis hysteresisVAVCoo[15](
      pre_y_start=true,
      uHigh=0.5,
      uLow=0)
      annotation (Placement(transformation(extent={{-80,102},{-62,120}})));
    Modelica.Blocks.Logical.Not not2[15]
      annotation (Placement(transformation(extent={{-56,84},{-46,94}})));
  equation
    ...
    connect(hysteresisHea.y, not1.u)
      annotation (Line(points={{-39,70},{-29.2,70}}, color={255,0,255}));
    connect(not1.y, heaDemand.u2) annotation (Line(points={{-15.4,70},{66,70},{
            66,38},{72,38}},            color={255,0,255}));
    ...
    connect(heaDemand.y, signalBus.Heacoils_signal) annotation (Line(points={{95,38},
            {100,38},{100,0}},       color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    ...
    connect(not1.y, or1.u[1:21]) annotation (Line(points={{-15.4,70},{8,70},{8,90},
            {20,90}},     color={255,0,255}));
    ...
    connect(abs[1].u, hysteresisVAVHea[1].u) annotation (Line(points={{-62,-26},{-66,
            -26},{-66,-8},{-94,-8},{-94,89},{-81.8,89}}, color={0,0,127}));
    connect(abs[2].u, hysteresisVAVHea[2].u);
    connect(abs[3].u, hysteresisVAVHea[3].u);
    connect(abs[5].u, hysteresisVAVHea[4].u);
    connect(abs[6].u, hysteresisVAVHea[5].u);
    connect(abs[7].u, hysteresisVAVHea[6].u);
    connect(abs[8].u, hysteresisVAVHea[7].u);
    connect(abs[9].u, hysteresisVAVHea[8].u);
    connect(abs[10].u, hysteresisVAVHea[9].u);
    connect(abs[11].u, hysteresisVAVHea[10].u);
    connect(abs[12].u, hysteresisVAVHea[11].u);
    connect(abs[13].u, hysteresisVAVHea[12].u);
    connect(abs[16].u, hysteresisVAVHea[13].u);
    connect(abs[17].u, hysteresisVAVHea[14].u);
    connect(abs[18].u, hysteresisVAVHea[15].u);
    ...
    connect(airDemand.u2, or2.y)
      annotation (Line(points={{-10,6},{-10,98},{-19.4,98}}, color={255,0,255}));
    connect(hysteresisVAVHea.u, hysteresisVAVCoo.u) annotation (Line(points={{-81.8,
            89},{-81.8,99.5},{-81.8,99.5},{-81.8,111}}, color={0,0,127}));
    connect(not2.u, hysteresisVAVHea.y) annotation (Line(points={{-57,89},{-59.5,89},
            {-59.5,89},{-61.1,89}}, color={255,0,255}));
    connect(not2.y, or2.u2) annotation (Line(points={{-45.5,89},{-40.75,89},{-40.75,
            93.2},{-33.2,93.2}}, color={255,0,255}));
  ...
  model AHU_nonGEOTABS "Controls of AHU loop for nonGEOTABS baseline"
    SignalBus signalBus annotation (Placement(transformation(extent={{80,-20},{
              120,20}}), iconTransformation(extent={{92,-10},{112,10}})));
    DataBus dataBus annotation (Placement(transformation(extent={{-120,-20},{
              -80,20}}), iconTransformation(extent={{-110,-10},{-90,10}})));
    ...
    Modelica.Blocks.Tables.CombiTable1D curve_AHU(table=[273.15 + 15,273.15 +
          ...
          273.15 + 20; 273.15 + 25,273.15 + 20.5; 273.15 + 26,273.15 + 19.5])
                              "Infrax AHU heating/cooling curve"
      annotation (Placement(transformation(extent={{-10,166},{10,186}})));
    Modelica.Blocks.Nonlinear.Limiter limiter(uMax=273.15 + 24, uMin=273.15 +
          18)
      annotation (Placement(transformation(extent={{20,166},{40,186}})));
    Modelica.Blocks.Logical.LessThreshold thermalWheelByPass(threshold=2)
      "thermal wheel by pass switch"
    ...
    IDEAS.Controls.Continuous.LimPID con3wayHea(
      ...
      Td=0,
      Ti(displayUnit="s") = 20,
      k=2)                 "Controller for 3-way valve of AHU heating coil"
      annotation (Placement(transformation(extent={{-8,-80},{12,-60}})));
    Modelica.Blocks.Math.BooleanToInteger P0809_signal
      "pump 08 and 09 signal, nominal conditions if heating is activated"
      annotation (Placement(transformation(extent={{20,-124},{40,-104}})));
    ...
    Components.Clock           clock
      annotation (Placement(transformation(extent={{-100,144},{-80,164}})));
    Modelica.Blocks.Sources.IntegerExpression P08P09signal(y=if (clock.hour >=
          6 and clock.hour <= 19) and (clock.weekDay) < 6 then P0809_signal.y
           else 0)
      "Pumps 08 and 09 signal"
      annotation (Placement(transformation(extent={{20,-100},{42,-80}})));
    Modelica.Blocks.Logical.Switch switch2
    ...
    Modelica.Blocks.Sources.BooleanExpression ahuCooDemand(y=if (clock.hour >=
          6 and clock.hour <= 19) and (clock.weekDay) < 6 and switch1.y > 0
           then true else false)
                       "AHU cooling demand"
      annotation (Placement(transformation(extent={{-10,-38},{12,-18}})));
    IDEAS.Buildings.Components.Interfaces.WeaBus weaBus(numSolBus=5)
      annotation (Placement(transformation(extent={{-72,160},{-52,180}})));
    Modelica.Blocks.Logical.Switch switch1
      annotation (Placement(transformation(extent={{52,22},{72,42}})));
  ...
  equation
    ...
    connect(or1.y, switch2.u2) annotation (Line(points={{15,108},{-2,108},{-2,
            84},{14,84}}, color={255,0,255}));
    connect(con3wayCoo.y, greaterThreshold1.u) annotation (Line(points={{11,48},
            {18,48},{18,22},{-32,22},{-32,2},{-12,2}}, color={0,0,127}));
    connect(ahuCooDemand.y, signalBus.ahuCooDem) annotation (Line(points={{13.1,
            -28},{100,-28},{100,0}}, color={255,0,255}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    ...
    connect(switch1.y, signalBus.CooCoi_3way_signal) annotation (Line(points={{
        ...
        horizontalAlignment=TextAlignment.Left));
    connect(switch3.y, signalBus.HeaCoi_3way_signal) annotation (Line(points={{
            81,-62},{90,-62},{90,-60},{100,-60},{100,0}}, color={0,0,127}),
        Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
    ...
    connect(closed.y, switch1.u3) annotation (Line(points={{32.6,-12},{42,-12},
            {42,24},{50,24}}, color={0,0,127}));
    connect(con3wayHea.y, switch3.u1)
      annotation (Line(points={{13,-70},{58,-70}}, color={0,0,127}));
    connect(closed.y, switch3.u3) annotation (Line(points={{32.6,-12},{46,-12},
            {46,-54},{58,-54}}, color={0,0,127}));
    connect(limiter.y, signalBus.T_AHU) annotation (Line(points={{41,176},{100,
            176},{100,0}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(limiter.y, con3wayCoo.u_s) annotation (Line(points={{41,176},{58,
            176},{58,200},{-72,200},{-72,48},{-12,48}}, color={0,0,127}));
    connect(curve_AHU.u[1], dataBus.Tavg_building) annotation (Line(points={{
            -12,176},{-99.9,176},{-99.9,0.1}}, color={0,0,127}), Text(
        string="%second",
        index=1,
    ...
    connect(con3wayCoo.u_s, con3wayTW.u_s) annotation (Line(points={{-12,48},{
            -72,48},{-72,110},{-28,110}}, color={0,0,127}));
    connect(dataBus.TAirSupplyAHU, con3wayTW.u_m) annotation (Line(
        points={{-99.9,0.1},{-99.9,90},{-16,90},{-16,98}},
        color={255,204,51},
        thickness=0.5), Text(
        string="%first",
        index=-1,
  ...
  model VAV_BOPTEST "Controls of AHU loop for baseline"
    SignalBus signalBus annotation (Placement(transformation(extent={{80,-20},{
              120,20}}), iconTransformation(extent={{92,-10},{112,10}})));
    DataBus dataBus annotation (Placement(transformation(extent={{-120,-20},{
              -80,20}}), iconTransformation(extent={{-110,-10},{-90,10}})));
    ...
    Modelica.Blocks.Sources.BooleanExpression AHUon(y=if ((clock.hour >= 6 and
          ...
          false) "Supply fan pressure difference set-point"
      annotation (Placement(transformation(extent={{10,100},{32,120}})));
    Modelica.Blocks.Sources.BooleanExpression trueSign[21](y=true)
      "Supply fan pressure difference set-point"
      annotation (Placement(transformation(extent={{-28,72},{-6,92}})));
    Modelica.Blocks.Sources.BooleanExpression trueSign1[15](y=true)
      "Supply fan pressure difference set-point"
      annotation (Placement(transformation(extent={{-36,56},{-14,76}})));
  equation
    connect(tracking.y, conVAV.u_s)
      annotation (Line(points={{-59,30},{-46,30}}, color={0,0,127}));
    connect(diff.y, abs.u)
    ...
    connect(limiter.y, max1.u) annotation (Line(points={{37,30},{38,30},{38,48},
            {12,48},{12,58},{18.4,58}}, color={0,0,127}));
    connect(AHUon.y, P06_signal.u) annotation (Line(points={{33.1,110},{48,110},
            {48,100},{62.4,100}}, color={255,0,255}));
    connect(trueSign.y, heaDemand.u2) annotation (Line(points={{-4.9,82},{34,82},
            {34,38},{72,38}}, color={255,0,255}));
    connect(airDemand.u2, trueSign1.y) annotation (Line(points={{-10,6},{-10,36},
            {-10,66},{-12.9,66}}, color={255,0,255}));
  ...
  model AHU_BOPTEST "Controls of AHU loop for baseline"
    SignalBus signalBus annotation (Placement(transformation(extent={{80,-20},{
              120,20}}), iconTransformation(extent={{92,-10},{112,10}})));
    DataBus dataBus annotation (Placement(transformation(extent={{-120,-20},{
              -80,20}}), iconTransformation(extent={{-110,-10},{-90,10}})));
    ...
    Components.Clock           clock
      annotation (Placement(transformation(extent={{-100,144},{-80,164}})));
    Modelica.Blocks.Sources.IntegerExpression P08P09signal(y=if (clock.hour >=
          6 and clock.hour <= 19) and (clock.weekDay) < 6
           then P0809_signal.y else 0)
      "Pumps 08 and 09 signal"
      annotation (Placement(transformation(extent={{20,-100},{42,-80}})));
    Modelica.Blocks.Sources.IntegerExpression P04signal(y=if (clock.hour >= 6
           and clock.hour <= 19) and (clock.weekDay) < 6
           then P04_signal.y else 0)
      "pump 04 signal"
      annotation (Placement(transformation(extent={{66,-48},{88,-28}})));
    Modelica.Blocks.Sources.RealExpression
                                     P13signal(y=if (clock.hour >= 6 and clock.hour
           <= 19) and (clock.weekDay) < 6 then P13_nom.y
           else 0)
      "Pump 13 signal"
      annotation (Placement(transformation(extent={{52,2},{74,22}})));
    Modelica.Blocks.Tables.CombiTable1D curve_HP(table=[273.15 + 15,273.15 + 24;
  ...
  equation
    ...
    connect(dataBus.TAirSupplyAHU, con3wayTW.u_m) annotation (Line(
        ...
        extent={{-3,-6},{-3,-6}},
        horizontalAlignment=TextAlignment.Right));
    connect(con3wayHea.y, signalBus.HeaCoi_3way_signal) annotation (Line(points={{
            13,-70},{50,-70},{50,-72},{100,-72},{100,0}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(con3wayCoo.y, signalBus.CooCoi_3way_signal) annotation (Line(points={{
            11,48},{40,48},{40,46},{100,46},{100,0}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(con3wayTW.y, signalBus.TW_bypass) annotation (Line(points={{-5,110},
            {12,110},{12,78},{100,78},{100,0}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
